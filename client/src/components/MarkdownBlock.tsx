...
const MARKDOWN_COMPONENTS = {
  h2: ({ children }: { children?: ReactNode }) => (
    <h2 className="blood-h2 mt-6 text-xl font-semibold font-display text-slate-100">
      {children ? renderWithHighlights(children) : null}
    </h2>
  ),
  h3: ({ children }: { children?: ReactNode }) => (
    <h3 className="mt-4 text-lg font-semibold font-display text-slate-100">
      {children ? renderWithHighlights(children) : null}
    </h3>
  ),
  ...
  li: ({ children }: { children?: ReactNode }) => (
    <li className="mt-2 text-sm leading-6 text-slate-300">
      {children ? renderWithHighlights(children) : null}
    </li>
  ),
