...
// Burnout Report Data - matches server structure
...
const PHASE_CONFIG = {
  ...
  resistance: {
    ...
    icon: Shield,
    description: "Surcharge compensatoire"
  },
  epuisement: {
    color: '#EF4444',
    ...
    icon: Flame,
    description: "Ressources epuisees"
  }
};

// Theme-aware score status
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      try {
        ...
        if (!data.globalScore || !data.sections || !data.metrics) {
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/70">Chargement de l'analyse burnout...</p>
        </div>
      </div>
    );
  }
      ...
      <main ref={mainContentRef} className="flex-1 overflow-y-auto relative z-10 scroll-smooth">
        ...
        <div className="lg:hidden sticky top-0 z-40 backdrop-blur-md px-4 py-4 flex items-center justify-between" style={{ backgroundColor: 'var(--color-bg)', borderBottom: `1px solid var(--color-border)` }}>
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm tracking-widest uppercase">Burnout Engine</span>
          </div>
          ...
          <div id="dashboard" className="pt-8 lg:pt-12">
            <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-12">
              <div className="space-y-6 max-w-2xl">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full" style={{ backgroundColor: phaseConfig.bgColor, border: `1px solid ${phaseConfig.color}30` }}>
                  <PhaseIcon className="w-4 h-4" style={{ color: phaseConfig.color }} />
                  <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: phaseConfig.color }}>{reportData.phaseLabel}</span>
                </div>
                <h1 className="text-5xl lg:text-7xl font-medium tracking-tighter leading-[0.9]">
                  {reportData.clientName}, <br />
                  <span style={{ color: currentTheme.colors.textMuted }}>ton analyse burnout.</span>
                </h1>
                <p className="text-lg leading-relaxed max-w-lg" style={{ color: 'var(--color-text-muted)' }}>
                  {reportData.globalScore}/100 de sante globale — {reportData.phase === 'epuisement' ? 'Action urgente requise.' : reportData.phase === 'resistance' ? 'Vigilance necessaire.' : 'Bons signaux de resilience.'}
                </p>
              </div>

              <div className="flex gap-4 items-end">
                <div className="text-right hidden md:block">
                  <div className="text-3xl font-bold font-mono">{reportData.globalScore}<span className="text-lg opacity-50">/100</span></div>
                  <div className="text-[10px] uppercase tracking-widest" style={{ color: 'var(--color-text-muted)' }}>Score Sante</div>
                </div>
              </div>
            </header>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="lg:col-span-1 lg:row-span-2 rounded-2xl p-8 flex flex-col justify-between relative overflow-hidden group" style={{ backgroundColor: 'var(--color-surface)', border: `1px solid var(--color-border)` }}>
                ...
                <h3 className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--color-text-muted)' }}>Niveau de Sante</h3>
                ...
                <div className="flex items-center justify-center gap-2">
                  <PhaseIcon className="w-4 h-4" style={{ color: phaseConfig.color }} />
                  <span className="text-xs font-medium px-3 py-1 rounded-full" style={{ backgroundColor: phaseConfig.bgColor, color: phaseConfig.color }}>
                    {reportData.phaseLabel}
                  </span>
                </div>
              </div>
              ...
              <div className="lg:col-span-4 rounded-2xl p-6 relative overflow-hidden flex flex-col md:flex-row gap-8 items-center" style={{ backgroundColor: 'var(--color-surface)', border: `1px solid var(--color-border)` }}>
                <div className="w-full md:w-1/3">
                  <h3 className="text-sm font-bold mb-2 flex items-center gap-2" style={{ color: phaseConfig.color }}>
                    <PhaseIcon size={16} /> Phase actuelle
                  </h3>
                  <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                    {reportData.phase === 'alarme' && "Ton corps reagit au stress mais conserve ses ressources. C'est le moment ideal pour agir."}
                    {reportData.phase === 'resistance' && "Tes surrenales compensent depuis un moment. Sans changement, tu risques l'epuisement."}
                    {reportData.phase === 'epuisement' && "Tes ressources sont tres faibles. Un repos profond et un accompagnement sont necessaires."}
                  </p>
                </div>
                <div className="w-full md:w-2/3 flex justify-center gap-4">
                  {(['alarme', 'resistance', 'epuisement'] as const).map((phase) => {
                    ...
                    return (
                      <div
                        key={phase}
                        className={`flex flex-col items-center gap-2 p-4 rounded-xl transition-all ${isActive ? 'ring-2 scale-105' : 'opacity-40'}`}
                        style={{
            ...
            <div className="max-w-2xl mx-auto">
              ...
              {reviewSubmitted ? (
                <div className="text-center p-8 rounded-2xl" style={{ backgroundColor: 'var(--color-surface)', border: `1px solid var(--color-border)` }}>
                  ...
                  <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                    {hasExistingReview
                      ? 'Tu as deja laisse un avis pour cette analyse.'
                ...
                <form onSubmit={handleSubmitReview} className="space-y-6 p-8 rounded-2xl" style={{ backgroundColor: 'var(--color-surface)', border: `1px solid var(--color-border)` }}>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Qu'as-tu pense de cette analyse burnout ? Ton avis nous aide a nous ameliorer..."
                      rows={4}
                      required
                      minLength={10}
          ...
          <footer className="py-24 flex flex-col md:flex-row justify-between items-start gap-8" style={{ borderTop: `1px solid var(--color-border)` }}>
            ...
            <div className="text-right">
              <p className="text-[10px] uppercase tracking-widest" style={{ color: 'var(--color-text-muted)' }}>
                Burnout Engine - {new Date(reportData.generatedAt).toLocaleDateString('fr-FR')}
              </p>
            </div>
          </footer>
        </div>
