...
export default function PeptidesPreviewPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setResult(payload.result); setCheckoutUrl(payload.checkoutUrl); trackFormSubmit("peptides_preview_completed"); window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  if (result) return <main className="min-h-screen bg-[#08090b] px-4 py-10 text-white sm:px-6">
    <div className="mx-auto max-w-4xl">
      <div className="mb-8 flex items-center justify-between"><a href="/offers/peptides-engine" className="text-sm font-black tracking-[.22em] text-white">APEX<span className="text-amber-400">LABS</span></a><span className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 text-xs text-emerald-300">PeptAura vérifié</span></div>
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-b from-white/[.075] to-white/[.025]">
        <div className="border-b border-white/10 p-6 sm:p-10"><p className="text-xs font-bold uppercase tracking-[.22em] text-amber-400">Aperçu pré-protocole de {form.firstName}</p><div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="max-w-2xl text-3xl font-semibold leading-tight sm:text-5xl">{result.headline}</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-zinc-400">{result.rationale}</p></div><div className="shrink-0 rounded-2xl bg-amber-400 p-5 text-center text-black"><span className="block text-5xl font-black">{result.moleculeCount}</span><span className="text-xs font-bold uppercase tracking-wider">molécule{result.moleculeCount > 1 ? "s" : ""}</span></div></div></div>
        ...
        <div className="border-t border-white/10 bg-black/25 p-6 sm:p-10"><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-white/10 p-5"><span className="text-xs uppercase tracking-wider text-zinc-500">Budget PeptAura initial estimé</span><strong className="mt-2 block text-3xl text-white">{result.estimatedStarterCostUsd == null ? "À valider" : `$${result.estimatedStarterCostUsd.toFixed(2)}`}</strong><span className="mt-2 block text-xs leading-5 text-zinc-500">Hors livraison. Estimation fondée sur le premier format achetable de chaque molécule, pas sur les quantités finales du cycle.</span></div><div className="rounded-2xl border border-white/10 p-5"><span className="text-xs uppercase tracking-wider text-zinc-500">Durée envisagée</span><strong className="mt-2 block text-lg text-white">{result.durationLabel}</strong><span className="mt-2 block text-xs text-zinc-500">Les dosages, quantités et arbitrages restent à calculer dans ton rapport complet.</span></div></div>
