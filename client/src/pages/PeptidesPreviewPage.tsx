  ...
  if (result) return <main className="min-h-screen bg-[#08090b] px-4 py-10 text-white sm:px-6">
    <div className="mx-auto max-w-4xl">
      ...
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-b from-white/[.075] to-white/[.025]">
        ...
        {result.molecules.length > 0 && <div className="grid gap-4 p-6 sm:p-10">{result.molecules.map((molecule, index) => <article key={molecule.name} className="rounded-2xl border border-white/10 bg-black/25 p-5"><div className="flex items-start justify-between gap-4"><div><span className="text-xs font-mono text-amber-400">0{index + 1}</span><h2 className="mt-1 text-xl font-semibold">{molecule.name}</h2><p className="mt-1 text-sm font-medium text-zinc-300">{molecule.role}</p></div><span className="whitespace-nowrap rounded-full bg-amber-400/10 px-3 py-1.5 text-xs font-bold text-amber-300">${molecule.estimatedTotalPriceUsd.toFixed(2)} total</span></div><div className="mt-4 rounded-xl border border-amber-400/15 bg-amber-400/[.045] p-4"><span className="text-[11px] font-bold uppercase tracking-[.14em] text-amber-400">Pourquoi ce choix</span><p className="mt-2 text-sm leading-6 text-zinc-300">{molecule.reason}</p></div><div className="mt-4 grid gap-3 rounded-xl border border-white/10 bg-white/[.025] p-4 text-xs leading-5 text-zinc-400 sm:grid-cols-2"><div><span className="block text-zinc-600">Durée chiffrée</span><strong className="text-zinc-200">{molecule.cycleDurationLabel}</strong></div><div><span className="block text-zinc-600">Besoin calculé</span><strong className="text-zinc-200">{molecule.totalRequiredMg} mg · {molecule.vialsRequired} fiole{molecule.vialsRequired > 1 ? "s" : ""} nécessaire{molecule.vialsRequired > 1 ? "s" : ""}</strong></div><div className="sm:col-span-2"><span className="block text-zinc-600">Base de calcul</span><strong className="text-zinc-200">{molecule.calculationBasis}</strong></div><div className="sm:col-span-2"><span className="block text-zinc-600">Commande réelle et calcul du prix</span><strong className="text-zinc-200">{molecule.packageCount} boîte{molecule.packageCount > 1 ? "s" : ""} × ${molecule.startingPackagePriceUsd.toFixed(2)} = ${molecule.estimatedTotalPriceUsd.toFixed(2)}</strong><span className="mt-1 block text-zinc-500">Format : {molecule.startingFormat} · {molecule.vialsPurchased} fiole{molecule.vialsPurchased > 1 ? "s" : ""} reçue{molecule.vialsPurchased > 1 ? "s" : ""}.</span>{molecule.vialsPurchased !== molecule.vialsRequired && <span className="mt-1 block text-amber-300">Le conditionnement impose {molecule.vialsPurchased - molecule.vialsRequired} fiole{molecule.vialsPurchased - molecule.vialsRequired > 1 ? "s" : ""} de marge ; elles sont incluses dans le prix.</span>}</div></div></article>)}</div>}
        <div className="border-t border-white/10 bg-black/25 p-6 sm:p-10"><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-white/10 p-5"><span className="text-xs uppercase tracking-wider text-zinc-500">Coût total molécules estimé</span><strong className="mt-2 block text-3xl text-white">{result.estimatedProtocolCostUsd == null ? "À valider" : `$${result.estimatedProtocolCostUsd.toFixed(2)}`}</strong><span className="mt-2 block text-xs leading-5 text-zinc-500">Hors livraison. Total calculé sur les boîtes réellement nécessaires pour les durées affichées, jamais sur une seule boîte de départ.</span></div><div className="rounded-2xl border border-white/10 p-5"><span className="text-xs uppercase tracking-wider text-zinc-500">Commande estimée</span><strong className="mt-2 block text-lg text-white">{result.totalVialsPurchased == null ? "À valider" : `${result.totalVialsPurchased} fioles reçues · ${result.totalPackages} boîtes`}</strong><span className="mt-2 block text-xs text-zinc-500">Besoin calculé : {result.totalVialsRequired ?? "à valider"} fioles. {result.durationLabel}.</span></div></div>
          <div className={`mt-4 rounded-xl border px-4 py-3 text-sm leading-6 ${result.budgetFit === "above" ? "border-amber-400/25 bg-amber-400/10 text-amber-200" : "border-emerald-400/20 bg-emerald-400/[.07] text-emerald-200"}`}>{result.budgetExplanation}</div>
          ...
          <div className="mt-4 flex items-center justify-center gap-2 text-center text-xs text-zinc-600"><LockKeyhole className="h-3.5 w-3.5" />Le calendrier opérationnel, la reconstitution et les ajustements individuels restent détaillés dans le rapport complet.</div>
        </div>
      </motion.section>
    </div>
  </main>;
