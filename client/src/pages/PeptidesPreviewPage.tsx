  ...
  if (result) return <main className="min-h-screen bg-[#08090b] px-4 py-10 text-white sm:px-6">
    <div className="mx-auto max-w-4xl">
      ...
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-b from-white/[.075] to-white/[.025]">
        ...
        {result.molecules.length > 0 && <div className="grid gap-4 p-6 sm:p-10">{result.molecules.map((molecule, index) => <article key={molecule.name} className="rounded-2xl border border-white/10 bg-black/25 p-5"><div className="flex items-start justify-between gap-4"><div><span className="text-xs font-mono text-amber-400">0{index + 1}</span><h2 className="mt-1 text-xl font-semibold">{molecule.name}</h2><p className="mt-1 text-sm font-medium text-zinc-300">{molecule.role}</p></div><span className="whitespace-nowrap rounded-full bg-amber-400/10 px-3 py-1.5 text-xs font-bold text-amber-300">${molecule.estimatedTotalPriceUsd.toFixed(2)} total</span></div><p className="mt-4 text-sm leading-6 text-zinc-400">{molecule.reason}</p><div className="mt-4 grid gap-2 rounded-xl border border-white/10 bg-white/[.025] p-4 text-xs leading-5 text-zinc-400 sm:grid-cols-2"><div><span className="block text-zinc-600">Durée chiffrée</span><strong className="text-zinc-200">{molecule.cycleDurationLabel}</strong></div><div><span className="block text-zinc-600">Quantité à commander</span><strong className="text-zinc-200">{molecule.vialsRequired} fiole{molecule.vialsRequired > 1 ? "s" : ""} · {molecule.packageCount} boîte{molecule.packageCount > 1 ? "s" : ""}</strong></div><div className="sm:col-span-2"><span className="block text-zinc-600">Hypothèse de chiffrage</span><strong className="text-zinc-200">{molecule.calculationBasis}</strong></div><div className="sm:col-span-2"><span className="block text-zinc-600">Format catalogue live</span><strong className="text-zinc-200">{molecule.startingFormat} à ${molecule.startingPackagePriceUsd.toFixed(2)} la boîte</strong>{molecule.vialsPurchased !== molecule.vialsRequired && <span className="mt-1 block text-amber-300">Conditionnement réel : {molecule.vialsPurchased} fioles reçues.</span>}</div></div></article>)}</div>}
        <div className="border-t border-white/10 bg-black/25 p-6 sm:p-10"><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-white/10 p-5"><span className="text-xs uppercase tracking-wider text-zinc-500">Coût total molécules estimé</span><strong className="mt-2 block text-3xl text-white">{result.estimatedProtocolCostUsd == null ? "À valider" : `$${result.estimatedProtocolCostUsd.toFixed(2)}`}</strong><span className="mt-2 block text-xs leading-5 text-zinc-500">Hors livraison. Total calculé sur les fioles et boîtes nécessaires pour les durées affichées, jamais sur une seule boîte de départ.</span></div><div className="rounded-2xl border border-white/10 p-5"><span className="text-xs uppercase tracking-wider text-zinc-500">Commande estimée</span><strong className="mt-2 block text-lg text-white">{result.totalVialsRequired == null ? "À valider" : `${result.totalVialsRequired} fioles · ${result.totalPackages} boîtes`}</strong><span className="mt-2 block text-xs text-zinc-500">{result.durationLabel}. Les hypothèses de calcul sont détaillées molécule par molécule.</span></div></div>
