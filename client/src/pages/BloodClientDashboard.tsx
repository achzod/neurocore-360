...
export default function BloodClientDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const scores = completedTests.map((t) => t.globalScore || 0);
      ...
      <main className="mx-auto max-w-6xl px-6 py-10 space-y-10">
        <section className="space-y-4">
          <div>
            ...
            <h1 className="text-3xl font-semibold">Injecter un bilan</h1>
          </div>
          ...
          <Card className="border border-white/10 bg-white/5 p-6 space-y-4">
            ...
            >
              <p className="text-white/70">Glisse un PDF ici</p>
              ...
              />
              {file && (
                <Badge className="mt-3 bg-white/10 text-white">{file.name}</Badge>
              )}
            </div>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <Card className="border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-white/50">Analyses totales</p>
            <p className="text-2xl font-semibold mt-2">{stats.total}</p>
          </Card>
          <Card className="border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-white/50">Score moyen</p>
            <p className="text-2xl font-semibold mt-2">{stats.avg}</p>
          </Card>
          <Card className="border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-white/50">Credits restants</p>
            <p className="text-2xl font-semibold mt-2">{credits}</p>
          </Card>
        </section>

        <section className="space-y-4">
          <div>
            <h2 className="text-xl font-semibold">Historique des bilans</h2>
            <p className="text-sm text-white/50">Clique un rapport termine pour l'ouvrir.</p>
          </div>

          <Card className="border border-white/10 bg-white/5 divide-y divide-white/10">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Badge className="bg-white/10 text-white uppercase">{test.status}</Badge>
            ...
            {(!tests || tests.bloodTests.length === 0) && (
              <div className="px-4 py-6 text-sm text-white/50">Aucun bilan pour l'instant.</div>
            )}
          </Card>
        </section>
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="text-sm text-white/50">Evolution du score global.</p>
          </div>

          {completedTests.length >= 2 ? (
            <Card className="border border-white/10 bg-white/5 p-4">
              <div className="flex items-center justify-between">
                <p className="text-sm text-white/60">Trajectoire</p>
