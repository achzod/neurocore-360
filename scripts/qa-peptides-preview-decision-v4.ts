...
const results = scenarios.map((scenario) => {
  ...
  const reserveMissing = result.molecules.some((molecule) => molecule.safetyReserveVials < 1 || molecule.vialsRequired <= molecule.operationalVials || molecule.vialsPurchased < molecule.vialsRequired);
