...
const results = scenarios.map((scenario) => {
  ...
  const durationMissing = !/12 semaines/.test(result.durationLabel) || !/actives/.test(result.durationLabel);
  if (result.status === "review_required" && (result.moleculeCount < 2 || result.moleculeCount > 4 || typeof result.estimatedGrandTotalUsd !== "number" || durationMissing || result.totalVialsRequired == null || reserveMissing)) failures.push("personalized estimate missing 2-4 molecules, explicit strategy/active duration, reserve vials or cost");
  if (result.status === "eligible" && (result.molecules.length < 2 || result.molecules.length > 4 || durationMissing || result.totalVialsRequired == null || reserveMissing || result.molecules.some((molecule) => molecule.calculationBasis.length < 20))) failures.push("eligible estimate missing 2-4 molecules, explicit strategy/active duration, math or reserve vials");
