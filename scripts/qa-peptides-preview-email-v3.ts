...
for (const [index, row] of checkpoint.results.entries()) {
  ...
  const clientHasEstimate = client.text.includes(`Nombre de molécules estimé : ${result.moleculeCount}`) && client.text.includes(`Durée de la stratégie : ${result.durationLabel}`) && (result.estimatedProtocolCostUsd == null || client.text.includes(`Produits : $${result.estimatedProtocolCostUsd.toFixed(2)}`)) && (result.estimatedShippingCostUsd == null || client.text.includes(`Livraison (une seule fois pour la commande complète) : $${result.estimatedShippingCostUsd.toFixed(2)}`)) && (result.estimatedGrandTotalUsd == null || client.text.includes(`Total rendu estimé : $${result.estimatedGrandTotalUsd.toFixed(2)}`));
