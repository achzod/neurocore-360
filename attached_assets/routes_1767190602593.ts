...
export async function registerRoutes(
        ...
        if (report.txt) {
          ...
          const mappedReport = {
            ...
            sections: dashboard.sections.map(s => ({
              id: s.id,
              title: s.title,
              score: 0, // Les scores par section ne sont plus dans le TXT
              introduction: s.content, // On met tout dans introduction pour l'instant
              isPremium: true
            })),
  ...
  app.get("/api/audits/:id/export/html", async (req, res) => {
    try {
      ...
      const html = generateExportHTML(narrativeReport, auditId);
