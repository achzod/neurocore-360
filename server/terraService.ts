  ...
  try {
    const response = await fetch(`${TERRA_CONFIG.BASE_URL}/auth/generateWidgetSession`, {
      ...
      body: JSON.stringify({
        ...
        auth_success_redirect_url: redirectUrl || `${process.env.BASE_URL || process.env.RENDER_EXTERNAL_URL || "https://apexlabs.onrender.com"}/audit-complet/questionnaire?terra_success=true`,
        auth_failure_redirect_url: `${process.env.BASE_URL || process.env.RENDER_EXTERNAL_URL || "https://apexlabs.onrender.com"}/audit-complet/questionnaire?terra_error=true`,
      }),
    });
