...
// Discovery Scan: Score gauge with domain arcs
const DiscoveryVisual: React.FC = () => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      <svg viewBox="0 0 300 200" width={400} height={280}>
        {/* Background arc */}
      ...
      {/* Domain pills */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center", maxWidth: 400, marginTop: 16 }}>
        {domains.map((d, i) => {
          const delay = i * 5;
          const pillOpacity = interpolate(frame, [delay + 20, delay + 30], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
          return (
            <div key={i} style={{ fontFamily: FONTS.mono, fontSize: 10, color: COLORS.yellow, opacity: pillOpacity, padding: "4px 10px", border: `1px solid ${COLORS.yellow}40`, borderRadius: 2, letterSpacing: "0.05em" }}>
              {d.toUpperCase()}
            </div>
          );
        })}
...
const AnabolicVisual: React.FC = () => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 40px", gap: 16 }}>
      {bars.map((bar, i) => {
        ...
        return (
          <div key={i}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <span style={{ fontFamily: FONTS.mono, fontSize: 12, color: "rgba(255,255,255,0.6)", letterSpacing: "0.1em" }}>{bar.label}</span>
              <span style={{ fontFamily: FONTS.mono, fontSize: 12, color: bar.color, fontWeight: 700 }}>{Math.round(width)}%</span>
            </div>
...
// Blood Analysis: DNA helix + biomarker labels
const BloodVisual: React.FC = () => {
  const frame = useCurrentFrame();
  const markers = ["CRP", "TSH", "HbA1c", "Ferritine", "Vitamine D", "LDL"];
  const numPairs = 8;
...
// Ultimate Scan: Body silhouette with scan
const UltimateVisual: React.FC = () => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
      <svg viewBox="0 0 400 800" width={240} height={480} style={{ overflow: "visible" }}>
        {/* Body silhouette */}
        <path
      ...
      {/* Wearables connected badges */}
      {[
        { text: "⌚ APPLE WATCH", x: "10%", y: "30%" },
        { text: "💍 OURA RING", x: "10%", y: "42%" },
        { text: "📱 WHOOP", x: "10%", y: "54%" },
      ].map((w, i) => {
        const wOpacity = interpolate(frame, [45 + i * 8, 55 + i * 8], [0, 0.85], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
        const wX = interpolate(frame, [45 + i * 8, 55 + i * 8], [-20, 0], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
        const pulse = Math.sin((frame + i * 12) / 10) * 0.1 + 0.9;
        return (
          <div
            key={`w-${i}`}
            style={{
      ...
      <div
        style={{
          ...
          color: COLORS.yellow,
          letterSpacing: "0.2em",
          opacity: interpolate(frame, [40, 50], [0, 0.6], { extrapolateRight: "clamp", extrapolateLeft: "clamp" }),
        }}
      >
        WEARABLES CONNECTÉS
      </div>
      ...
      {/* Data overlays */}
      {[
        { text: "HRV: 142ms", x: "75%", y: "25%" },
        { text: "Sleep: 94%", x: "75%", y: "40%" },
        { text: "VO2: 48", x: "75%", y: "55%" },
      ].map((d, i) => (
        <div
          key={i}
          style={{
...
// FormCheck: Stick figure with angles
const FormCheckVisual: React.FC = () => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative" }}>
      <svg viewBox="0 0 200 260" width={180} height={240}>
      ...
      {/* Exercise tiles */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "center", maxWidth: 360, marginTop: 12 }}>
        {exercises.map((ex, i) => {
          const pulse = Math.sin((frame + i * 8) / 8) * 0.3 + 0.7;
          return (
            <div key={i} style={{ fontFamily: FONTS.mono, fontSize: 9, color: COLORS.green, opacity: interpolate(frame, [50 + i * 3, 55 + i * 3], [0, pulse], { extrapolateRight: "clamp", extrapolateLeft: "clamp" }), padding: "3px 8px", border: `1px solid ${COLORS.green}40`, borderRadius: 2 }}>
              {ex}
            </div>
          );
        })}
...
const ProductSlide: React.FC<{ product: Product; visual: React.ReactNode }> = ({ product, visual }) => {
  ...
  const textX = interpolate(slideIn, [0, 1], [-80, 0]);
  const textOpacity = interpolate(slideIn, [0, 1], [0, 1]);
  const visualX = interpolate(slideIn, [0, 1], [80, 0]);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg }}>
      ...
      <HUDCorners color={product.accentColor} padding={30} size={30} />

      <div style={{ display: "flex", height: "100%", alignItems: "center", padding: "0 100px" }}>
        {/* Left: Text */}
        <div style={{ flex: "0 0 55%", transform: `translateX(${textX}px)`, opacity: textOpacity }}>
          {/* Watermark number */}
          <div style={{ fontFamily: FONTS.mono, fontSize: 200, fontWeight: 900, color: "rgba(255,255,255,0.03)", position: "absolute", top: 60, left: 60, lineHeight: 1 }}>
            {product.number}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
            ...
            <span style={{ fontFamily: FONTS.mono, fontSize: 13, color: product.accentColor, letterSpacing: "0.15em", textTransform: "uppercase" }}>
              {product.subtitle}
            </span>
          </div>
          ...
          {nameLines.map((line, i) => (
            <div key={i} style={{ fontFamily: FONTS.heading, fontSize: 72, fontWeight: 900, color: i === 1 ? product.accentColor : COLORS.white, letterSpacing: "-0.04em", lineHeight: 0.95 }}>
              {line}
            </div>
          ))}

          {/* Features */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 24px", marginTop: 32 }}>
            {product.features.map((feat, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, opacity: interpolate(frame, [15 + i * 5, 25 + i * 5], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" }) }}>
                <span style={{ fontFamily: FONTS.mono, fontSize: 14, color: product.accentColor, fontWeight: 700 }}>&gt;</span>
                <span style={{ fontFamily: FONTS.mono, fontSize: 13, color: "rgba(255,255,255,0.7)", letterSpacing: "0.03em" }}>{feat}</span>
              </div>
            ))}
          </div>

          {/* Price */}
          <div style={{ marginTop: 32, display: "inline-block", padding: "12px 24px", border: `1px solid ${product.accentColor}30`, backgroundColor: `${product.accentColor}0A` }}>
            <div style={{ fontFamily: FONTS.mono, fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 4 }}>
              Investissement
            </div>
            <div style={{ fontFamily: FONTS.heading, fontSize: 36, fontWeight: 900, color: COLORS.white, letterSpacing: "-0.02em" }}>
              {product.price}
            </div>
          </div>
        </div>
        ...
        {/* Right: Visual */}
        <div style={{ flex: "0 0 45%", height: "70%", transform: `translateX(${visualX}px)`, opacity: textOpacity, position: "relative", borderRadius: 4, overflow: "hidden", border: `1px solid ${product.accentColor}20` }}>
          {visual}
        </div>
      </div>
    </AbsoluteFill>
...
export const Scene04_Products: React.FC = () => {
  return (
    <AbsoluteFill>
